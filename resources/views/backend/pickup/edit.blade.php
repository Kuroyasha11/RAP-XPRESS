...
@section('container')
    ...
    <div class="col-lg-15">
        <form action="/dashboard/pickup/{{ $pickup->id }}" method="post">
            @method('put')
            @csrf
            <div class="mb-3">
                <label for="nama" class="form-label">Kode Kurir</label>
                <input type="text" class="form-control @error('nama') is-invalid @enderror" name="nama" id="nama"
                    placeholder="Kode Kurir" value="{{ old('nama', $pickup->nama) }}" autofocus required>
                @error('nama')
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('user_id') is-invalid @enderror" name="user_id">
                    @foreach ($users as $user)
                        @if (old('user_id', $pickup->user_id) == $user->id)
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('alamat') is-invalid @enderror" name="alamat" id="alamat"
                    placeholder="Daerah yang diambil" value="{{ old('alamat', $pickup->alamat) }}" required>
                @error('alamat')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
