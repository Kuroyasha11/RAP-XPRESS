...
@section('container')
    ...
    <div class="col-lg-5">
        ...
        @if ($kurir->titik1 == 0 && $kurir->titik2 == 0 && $kurir->selesai == 0)
            ...
            <form action="/dashboard/kurir/kurir/{{ $kurir->id }}" class="d-inline" method="post">
                @method('put')
                @csrf
            ...
            <p class="text-muted">Terima apabila Paket siap diantar oleh Kurir</p>
        @elseif ($kurir->titik1 == 1 && $kurir->titik2 == 0 && $kurir->selesai == 0)
            <form action="/dashboard/kurir/kurir/{{ $kurir->id }}" class="d-inline" method="post">
                @method('put')
                @csrf
            ...
            <p class="text-muted">Batalkan apabila Paket tidak jadi diantar oleh Kurir</p>
        @endif

        @if ($kurir->titik1 == 1 && $kurir->titik2 == 1 && $kurir->selesai == 0)
            <form action="/dashboard/kurir/selesai/{{ $kurir->id }}" class="d-inline" method="post">
                @method('put')
                @csrf
                ...
                @if ($packages->count())
                    @foreach ($packages as $package)
                        <tr>
                            ...
                            <td>
                                @if ($kurir->titik1 == 0 && $kurir->titik2 == 0 && $kurir->selesai == 0)
                                    <form action="/dashboard/kurir/paket/delete/{{ $package->slug }}" method="post"
                                        class="d-inline">
                                        @method('put')
                                        @csrf
                                ...
                                @elseif ($package->diambil == 1 && $package->selesai == 1)
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="/dashboard/kurir/paket/{{ $kurir->id }}" method="post">
                        @method('put')
                        @csrf
                        <div class="mb-3">
