...
@section('container')
    ...
    <div class="col-lg-5">
        <a href="/dashboard/pengambilan-paket" class="btn btn-success"><i class="bi bi-arrow-left-circle"></i> Back</a>

        @if ($pickup->titik1 == 1 && $pickup->titik2 == 0 && $pickup->selesai == 0)
            <form action="/dashboard/pengambilan-paket/{{ $pickup->id }}" class="d-inline" method="post">
                @method('put')
                @csrf
                ...
                @if ($packages->count())
                    @foreach ($packages as $package)
                        <tr>
                            ...
                            <td>
                                <a href="/dashboard/pengambilan-paket/paket/{{ $package->slug }}"
